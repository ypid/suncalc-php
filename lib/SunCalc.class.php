...
class SunCalc {
	...
	static $version_hash = "01f836cac9bd17455447d80bfab31b666fa392d5";
	...
	static function toJulian($date) {
		return $date->getTime() / SunCalc::$dayMs - 0.5 + SunCalc::$J1970;
	}
	static function fromJulian($j) {
		return Date::fromTime(($j + 0.5 - SunCalc::$J1970) * SunCalc::$dayMs);
	}
	static function toDays($date) {
		return $date->getTime() / SunCalc::$dayMs - 0.5 + SunCalc::$J1970 - SunCalc::$J2000;
	}
	...
	static function eclipticLongitude($M) {
		$C = SunCalc::$rad * (1.9148 * Math::sin($M) + 0.02 * Math::sin(2 * $M) + 0.0003 * Math::sin(3 * $M));
		$P = SunCalc::$rad * 102.9372;
		return $M + $C + $P + Math::$PI;
	}
	static function sunCoords($d) {
		$M = SunCalc::solarMeanAnomaly($d);
		$L = SunCalc::eclipticLongitude($M);
		return _hx_anonymous(array("dec" => SunCalc::declination($L, 0), "ra" => SunCalc::rightAscension($L, 0)));
	}
	static function getPosition($date, $lat, $lng) {
		...
		return _hx_anonymous(array("azimuth" => SunCalc::azimuth($H, $phi, $c->dec), "altitude" => SunCalc::altitude($H, $phi, $c->dec)));
	}
	static $times;
	static function addTime($angle, $riseName, $setName) {
		SunCalc::$times->push((new _hx_array(array($angle, $riseName, $setName))));
	}
	static $J0 = 0.0009;
	...
	static function getTimes($date, $lat, $lng) {
		...
		$phi = SunCalc::$rad * $lat;
		$d = SunCalc::toDays($date);
		$n = Math::round($d - SunCalc::$J0 - $lw / (2 * Math::$PI));
		...
		{
			$v = SunCalc::fromJulian($Jnoon);
			$result->set("solarNoon", $v);
			$v;
		}
		{
			$v1 = SunCalc::fromJulian($Jnoon - 0.5);
			$result->set("nadir", $v1);
			$v1;
		}
		{
			...
			while($_g < $_g1->length) {
				$time1 = $_g1[$_g];
				++$_g;
				$Jset = SunCalc::getSetJ($time1->a[0] * SunCalc::$rad, $lw, $phi, $dec, $n, $M, $L);
				$Jrise = $Jnoon - ($Jset - $Jnoon);
				{
					$k = $time1[1];
					$v2 = SunCalc::fromJulian($Jrise);
					$result->set($k, $v2);
					$v2;
					unset($v2,$k);
				}
	...
	static function moonCoords($d) {
		$L = SunCalc::$rad * (218.316 + 13.176396 * $d);
		$M = SunCalc::$rad * (134.963 + 13.064993 * $d);
		$F = SunCalc::$rad * (93.272 + 13.229350 * $d);
		$longitude = $L + SunCalc::$rad * 6.289 * Math::sin($M);
		$latitude = SunCalc::$rad * 5.128 * Math::sin($F);
		$dt = 385001 - 20905 * Math::cos($M);
		return _hx_anonymous(array("ra" => SunCalc::rightAscension($longitude, $latitude), "dec" => SunCalc::declination($longitude, $latitude), "dist" => $dt));
	}
	static function getMoonPosition($date, $lat, $lng) {
		...
		$h = $h + SunCalc::$rad * 0.017 / Math::tan($h + SunCalc::$rad * 10.26 / ($h + SunCalc::$rad * 5.10));
		return _hx_anonymous(array("azimuth" => SunCalc::azimuth($H, $phi, $c->dec), "altitude" => $h, "distance" => $c->dist));
	}
	...
	static function hoursLater($date, $h) {
		return Date::fromTime($date->getTime() + $h * SunCalc::$dayMs / 24);
	}
	static function getMoonTimes($date, $lat, $lng, $inUTC = null) {
		...
		if($inUTC) {
			{
				$this1 = datetime__DateTime_DateTime_Impl_::utc(SunCalc_0($date, $inUTC, $lat, $lng));
		...
		$hc = 0.133 * SunCalc::$rad;
		$h0 = SunCalc::getMoonPosition($date, $lat, $lng)->altitude - $hc;
		$h1 = null;
		$h2 = null;
		$rise = 0.0;
		$set = 0.0;
		...
		$i = 1;
		while($i <= 24) {
			$h1 = SunCalc::getMoonPosition(SunCalc::hoursLater($date, $i), $lat, $lng)->altitude - $hc;
			$h2 = SunCalc::getMoonPosition(SunCalc::hoursLater($date, $i + 1), $lat, $lng)->altitude - $hc;
			$a = ($h0 + $h2) / 2 - $h1;
			$b = ($h2 - $h0) / 2;
			$xe = -$b / (2 * $a);
			$ye = ($a * $xe + $b) * $xe + $h1;
		...
		$result = new haxe_ds_StringMap();
		if(!_hx_equal($rise, 0)) {
			$v = SunCalc::hoursLater($date, $rise);
			$result->set("rise", $v);
			$v;
		}
		if(!_hx_equal($set, 0)) {
			$v1 = SunCalc::hoursLater($date, $set);
			$result->set("set", $v1);
			$v1;
		}
		if(_hx_equal($rise, 0) && _hx_equal($set, 0)) {
	...
	function __toString() { return 'SunCalc'; }
}
SunCalc::$rad = Math::$PI / 180;
SunCalc::$e = Math::$PI / 180 * 23.4397;
SunCalc::$times = (new _hx_array(array((new _hx_array(array(-0.833, "sunrise", "sunset"))), (new _hx_array(array(-0.3, "sunriseEnd", "sunsetStart"))), (new _hx_array(array(-6, "dawn", "dusk"))), (new _hx_array(array(-12, "nauticalDawn", "nauticalDusk"))), (new _hx_array(array(-18, "nightEnd", "night"))), (new _hx_array(array(6, "goldenHourEnd", "goldenHour"))))));
function SunCalc_0(&$date, &$inUTC, &$lat, &$lng) {
	{
